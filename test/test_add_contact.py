...
def test_add_contact_wo_group(app, db, json_contacts, check_ui):
    ...
    if check_ui:
        assert sorted(old_contacts_list, key=Contact.id_or_max) == sorted(app.contact.get_contact_list(), key=Contact.id_or_max)